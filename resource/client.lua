			...
			options = {
				{
					...
					canInteract = function(entity)
						return not IsEntityDead(entity) and not IsEntityPositionFrozen(entity) and does_have_drugs() and state_check(entity) and GetPedType(entity) ~= 28
					end
				},
			},
			distance = 5,
	...
	if Config.interaction.type == '3dtext' then
		...
		Citizen.CreateThread(function()
			while true do
				...
				if closestPed ~= nil and not IsEntityDead(closestPed) and not IsEntityPositionFrozen(closestPed) and does_have_drugs() and state_check(closestPed) and GetPedType(closestPed) ~= 28 then

					
	
					while closestPed ~= nil and not IsEntityDead(closestPed) and not IsEntityPositionFrozen(closestPed) and state_check(closestPed) and not IsPedInAnyVehicle(closestPed, true) do
...
function state_check(closestPed)
